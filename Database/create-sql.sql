...
-- Wednesday, April 03, 2019 PM01:51:23 CST
...
CREATE TABLE IF NOT EXISTS `OnlineJudge`.`solutions` (
  ...
  CONSTRAINT `fk_solution_problem`
    ...
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_solution_contest_problem`
    FOREIGN KEY (`contest_problem_id`)
    REFERENCES `OnlineJudge`.`contest_problems` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_solutions_contest_id`
    FOREIGN KEY (`contest_id`)
    REFERENCES `OnlineJudge`.`contests` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
...
-- -----------------------------------------------------
-- Table `OnlineJudge`.`solution_ full_results`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `OnlineJudge`.`solution_ full_results` ;

SHOW WARNINGS;
CREATE TABLE IF NOT EXISTS `OnlineJudge`.`solution_ full_results` (
  `solution_id` INT UNSIGNED NOT NULL,
  `created_at` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
  `updated_at` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
